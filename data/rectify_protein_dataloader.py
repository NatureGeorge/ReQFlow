...
class Rectify_ProteinData(LightningDataModule):
    ...
    def train_dataloader(self, rank=None, num_replicas=None):
        ...
        return DataLoader(
            self._train_dataset,
            batch_sampler=RectifyLengthBatcher(
...
class RectifyLengthBatcher:
    ...
    def __init__(
        ...
        #* Force the data to be samples only, and that is the index of pdb_name.
        self._data_csv = metadata_csv[metadata_csv['type'] == 'sample']
        self._data_csv = self._data_csv.reset_index(drop=True)
        self._data_csv['index'] = range(len(self._data_csv))
...
class LengthBatcher:
    ...
    def _replica_epoch_batches(self):
        ...
        # Each batch contains multiple proteins of the same length.
        sample_order = []
        for seq_len, len_df in replica_csv.groupby('length'):
            max_batch_size = min(
                self.max_batch_size,
                self._sampler_cfg.max_num_res_squared // seq_len**2 + 1,
            )
